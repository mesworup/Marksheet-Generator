...
def add_subject_row(frame, s_entries, m_entries):
    ...
    s.grid(row=row_index, column=0, padx=5, pady=4)
    m = ttk.Entry(frame, width=10)
    m.grid(row=row_index, column=1, padx=5, pady=4)
    s_entries.append(s)
    m_entries.append(m)
...
# Center card frame
card = ttk.Frame(root, padding=25, bootstyle=INFO)
card.place(relx=0.5, rely=0.5, anchor="center")

# School Title
ttk.Label(card, text="Kathmandu National School", font=("Segoe UI", 22, "bold")).grid(row=0, column=0, columnspan=4, pady=15)
...
# -------- Subjects Table with Headers --------
ttk.Label(card, text="Subjects & Marks", font=("Segoe UI", 14, "bold")).grid(row=4, column=0, columnspan=4, pady=15)
subject_frame = ttk.Frame(card, padding=10, bootstyle="light")
subject_frame.grid(row=5, column=0, columnspan=4, pady=5)

# Table headers
ttk.Label(subject_frame, text="Subject", font=("Segoe UI", 12, "bold"), width=25, anchor="center", relief="ridge").grid(row=0, column=0, padx=1, pady=1)
ttk.Label(subject_frame, text="Marks", font=("Segoe UI", 12, "bold"), width=10, anchor="center", relief="ridge").grid(row=0, column=1, padx=1, pady=1)
